...
1. `:HimalayaAi prompts` — interactive buffer with e/v/q keybinds
2. `:HimalayaAi validate` — reuse existing `run_ai` machinery with prompt picker
3. `:HimalayaAi set` for vault/save_dir/format (path validation + persist)
4. Config auto-reload on `:HimalayaAi edit` save

## Long-term (Future sessions)

1. Custom prompt creation (`:HimalayaAi add-prompt <name>`)
2. Prompt history (track which prompts work best)
3. Per-account prompt overrides (work vs personal email)
