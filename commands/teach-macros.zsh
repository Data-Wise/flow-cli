...
#   - Sync macros from source files to registry
...
# Purpose: Extract macros from source files and update registry
# =============================================================================
# Arguments:
...
_teach_macros_sync() {
    ...
    # Get previous macro count for diff (check both old and new filename)
    local prev_count=0
    if [[ -f ".flow/macros/registry.yml" ]]; then
        prev_count=$(command grep -c "^  [a-zA-Z]" ".flow/macros/registry.yml" 2>/dev/null || echo "0")
    elif [[ -f ".flow/macros/cache.yml" ]]; then
        prev_count=$(command grep -c "^  [a-zA-Z]" ".flow/macros/cache.yml" 2>/dev/null || echo "0")
        # Migrate: remove old filename (new one will be written below)
        rm -f ".flow/macros/cache.yml"
    fi
    ...
    # Write cache file
    local cache_file=".flow/macros/registry.yml"
    {
        echo "# LaTeX Macro Registry"
...
_teach_macros_sync_help() {
  ...
  WHAT IT DOES
    ...
    3. Writes merged macros to .flow/macros/registry.yml
...
${FLOW_COLORS[bold]}COMMANDS${FLOW_COLORS[reset]}
  ...
  ${FLOW_COLORS[accent]}sync${FLOW_COLORS[reset]}    Extract macros from sources to registry
...
${FLOW_COLORS[bold]}QUICK EXAMPLES${FLOW_COLORS[reset]}
  ...
  ${FLOW_COLORS[muted]}\$${FLOW_COLORS[reset]} teach macros sync          ${FLOW_COLORS[dim]}# Refresh registry${FLOW_COLORS[reset]}
