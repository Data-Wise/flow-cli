...
_doctor_check_zsh_plugin() {
  ...
  if $loaded; then
    echo "  ${FLOW_COLORS[success]}✓${FLOW_COLORS[reset]} $name ${FLOW_COLORS[muted]}(active)${FLOW_COLORS[reset]}"
    return 0
  elif $in_list; then
    echo "  ${FLOW_COLORS[warning]}△${FLOW_COLORS[reset]} $name ${FLOW_COLORS[muted]}(listed but not loaded - restart shell)${FLOW_COLORS[reset]}"
    return 1
  else
    echo "  ${FLOW_COLORS[muted]}○${FLOW_COLORS[reset]} $name ${FLOW_COLORS[muted]}(not configured)${FLOW_COLORS[reset]}"
    return 1
  fi
}
...
_doctor_check_plugin_manager() {
  echo "${FLOW_COLORS[bold]}🔌 PLUGIN MANAGER${FLOW_COLORS[reset]}"

  # Check antidote
  if command -v antidote >/dev/null 2>&1; then
    local version=$(antidote --version 2>/dev/null | head -1)
    echo "  ${FLOW_COLORS[success]}✓${FLOW_COLORS[reset]} antidote ${FLOW_COLORS[muted]}$version${FLOW_COLORS[reset]}"

    # Check bundle file
    local bundle_file="${ZDOTDIR:-$HOME/.config/zsh}/.zsh_plugins.txt"
    if [[ -f "$bundle_file" ]]; then
      local plugin_count=$(grep -v '^#' "$bundle_file" | grep -v '^$' | wc -l | tr -d ' ')
      echo "  ${FLOW_COLORS[success]}✓${FLOW_COLORS[reset]} plugins.txt ${FLOW_COLORS[muted]}($plugin_count plugins)${FLOW_COLORS[reset]}"
    else
      echo "  ${FLOW_COLORS[warning]}△${FLOW_COLORS[reset]} plugins.txt ${FLOW_COLORS[muted]}not found${FLOW_COLORS[reset]}"
    fi
  elif command -v zinit >/dev/null 2>&1; then
    echo "  ${FLOW_COLORS[success]}✓${FLOW_COLORS[reset]} zinit"
  elif [[ -d "$HOME/.oh-my-zsh" ]]; then
    echo "  ${FLOW_COLORS[success]}✓${FLOW_COLORS[reset]} oh-my-zsh"
  else
    echo "  ${FLOW_COLORS[muted]}○${FLOW_COLORS[reset]} No plugin manager detected"
  fi
  echo ""
}
